...
for (var i = 0; i <= 32; i++) {
  ...
  RF_input["RF" + i] = {freq: [0], phase: [0], amp: [0], time: [0], names: [{sequences:[""]}]}
...
export default class App extends React.Component {
    ...
    fetch(hardware_url).then(
      (response) => response.json()).then(
        (data) => {
          ...
          this.setState({rf_names_map: this.createRFMap(JSON.parse(data)["RFs"])})
          this.setState({TTL_names_map: this.createTTLMap(JSON.parse(data)["TTLs"])})
        }
      );
  }
  ...
  handleRFChange(e) {
    this.state.enabledChannels[e.name].enabled = !this.state.enabledChannels[e.name].enabled;
    this.setState(e);
  }
