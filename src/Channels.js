...
class RFScope extends Component{
  ...
  onClickAnnotation(e){
    let clickedChannel = Object.keys(this.props.rf_names_map).find(key => this.props.rf_names_map[key] === e.annotation.text);
  ...
  render(){
    ...
    let RF_input = filter(this.props.RF_input, Object.keys(this.props.rf_names_map));
    let enabled_RFs = filter(this.props.enabledChannels, Object.keys(this.props.rf_names_map));
    let n_RF_channels = Object.values(enabled_RFs).reduce((a, item) => a + item.enabled, 0)
    
    let enabled_TTLs = filter(this.props.enabledChannels, Object.keys(this.props.TTL_names_map));
    let n_TTL_channels = Object.values(enabled_TTLs).reduce((a, item) => a + item.enabled, 0) //Count number of enabled TTLs
    let TTL_input = filter(this.props.RF_input, Object.keys(this.props.TTL_names_map));
...
data.push(PMT_to_add);
  ...
  for (const [channel, value] of Object.entries(RF_input)) {
    if (this.props.enabledChannels[channel].enabled) {
      let object_to_add = Object.assign({}, data_templates[this.props.enabledChannels[channel].showing]);
      ...
      layout_to_use["yaxis" + index].title.text = this.props.enabledChannels[channel].showing;
      layout_to_use["yaxis" + index].range = RF_yaxis_ranges[this.props.enabledChannels[channel].showing];
